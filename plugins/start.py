from pyrogram import Client as ExistBots
...
@ExistBots.on_message(filters.command("status"))
def show_status(client: ExistBots, message: Message):
...
@ExistBots.on_message(filters.private & filters.command("broad"))
def broadcaster(c: ExistBots, m: Message):
...
@ExistBots.on_message(filters.incoming)
async def handle_message(existbots: ExistBots, message: Message):
    add_served_user(message.chat.id)
    if FORCER.FORCE_BOOL == True:
        FORCE = False
        try:
            got_chat_member: ChatMember = await existbots.get_chat_member(
            ...
            FORCE = True
        if FORCE == True:
            chat: Chat = await existbots.get_chat(FORCER.CHANNEL_USERNAME)
